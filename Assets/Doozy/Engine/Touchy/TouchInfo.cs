...
namespace Doozy.Engine.Touchy
{
    ...
    public struct TouchInfo
    {
        ...
        {
            if (Tap) return $"[Tap: At {StartPosition}, Time {Duration:0.00}s]";
            if (LongTap) return $"[LongTap: At {StartPosition}, Time {Duration:0.00}s]";
            return
                $"[Swipe: {Direction}, From {RawDirection}, To {StartPosition}, Delta {EndPosition}, Time {Duration:0.00}s]";
        }
    }
}