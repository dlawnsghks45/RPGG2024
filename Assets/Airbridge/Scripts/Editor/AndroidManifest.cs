...
public class AndroidManifest
{
    ...
    {
        foreach (var httpScheme in new[] { "http", "https" })
        {
        ...
        private XmlElement GetData(string scheme, string host = "", string path = "")
        {
            foreach (XmlElement element in data)
            {
                ...
                if (!string.IsNullOrEmpty(host))
                {
                    ...
                    foreach (var airbridgeAppLinksHost in new[] { "abr.ge", "airbridge.io", "deeplink.page" })
                    {
                        if (host.EndsWith(airbridgeAppLinksHost))
                        {
