...
public class CSV2Table : EditorWindow
{
	...
	void OnGUI()
	{
		...
		if(GUILayout.Button("Generate Code"))
		{
			...
			string path = "";
				path = EditorUtility.SaveFilePanel("Save Script", "Assets/Script/DB", csv.name + "DB.cs", "cs");
			script = CreateScript(csv, path);
			return;
			if(script != null)
			{
				//path = AssetDatabase.GetAssetPath(script);
				path = EditorUtility.SaveFilePanel("Save Script", "Assets/Resources/Csv", csv.name + "DB.cs", "cs");
			}
			else
			{
				path = EditorUtility.SaveFilePanel("Save Script", "Assets/Script/DB", csv.name + "DB.cs", "cs");
		...
		if (GUILayout.Button("Reset") && csv != null)
        {
			csv = null;
			script = null;
	...
	public static MonoScript CreateScript(TextAsset csv, string path)
	{
		...
		string code = TableCodeGen.Generate(csv.name,csv.text, className);
